...
function walkChildren(element) {
    ...
    for (i = 0; i < element.children.length; i++) {
        ...
        for (a = 0; a < child.attributes.length; a++) {
            ...
            if (EVENTS_BLOCKLIST.includes(attr.name)) {
                logToPage("      > <b>Found dangerous attribute '" + attr.name + "'.</b> Removing it from the '" + child.nodeName + "' element.")
